...
export const updateDonorProfile = async (req, res) => {
  try {
    ...
    const { fullName, phone, address, age, gender, weight, password } =
      req.body;
...
export const getDonorStats = async (req, res) => {
  try {
    ...
    if (lastDonationDate) {
      ...
      if (nextEligibleDonationDate > new Date()) {
        const remainingDays = Math.ceil(
          (nextEligibleDonationDate - new Date()) / (1000 * 60 * 60 * 24),
        );
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch donor statistics." });
  }
};
...
export const getDonorHistory = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch donation history." });
  }
};
...
export const getRecentDonations = async (req, res) => {
  try {
    ...
    const [todayDonations, weekDonations, allDonations, recentDonors] =
      await Promise.all([
        // Today's donations
